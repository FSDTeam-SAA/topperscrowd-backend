...
const cleanupPendingOrders = createCronTask('OrderReconciler', async (): Promise<ITaskResult> => {
  ...
  const pendingOrders = await Order.find({
    ...
    createdAt: { $lte: thresholdDate },
    paypalOrderId: { $exists: true, $ne: null },
  }).limit(20); // Cap it per run to prevent memory/rate-limit bottlenecks

  result.totalScanned = pendingOrders.length;
  if (pendingOrders.length === 0) return result;

  // Generate PayPal Access Token
  let accessToken: string, baseURL: string;
  try {
    const { clientId, clientSecret, mode } = config.paypal;
    baseURL = mode === 'sandbox' ? 'https://api-m.sandbox.paypal.com' : 'https://api-m.paypal.com';
    const auth = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');
  ...
  // 2. Process tasks concurrently to avoid blocking the single-threaded Event Loop
  const orderPromises = pendingOrders.map(async (order) => {
    try {
      ...
      // FIX FOR THE 404 INFINITE LOOP:
      // If PayPal says the order doesn't exist (404), cancel it locally so it is filtered out next run
      if (err.response && err.response.status === 404) {
        logger.warn(`[OrderReconciler] Order ${order._id} with PayPal ID ${order.paypalOrderId} not found on PayPal. Cancelling locally.`);
        order.paymentStatus = 'cancelled';
        await order.save();
      }
    }
